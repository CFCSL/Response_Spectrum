...
def download_abaqus(list_df, file_name):
    ...
    for i, df in enumerate(list_df, 1):
        ...
        # Create a string representation of the rounded DataFrame with columns separated by three spaces
        data_csv = df_rounded.to_string(index=False, header=False, sep=',')
        pattern = r'[A-Z]'
    ...
    buffer.seek(0)
     ...
     # Create the download link
    b64 = base64.b64encode(buffer.read()).decode()
    href = f'<a href="data:text/plain;base64,{b64}" download="{file_name}_RS_ABAQUS.txt">Download Abaqus Text</a>'
    st.markdown(href, unsafe_allow_html=True)
